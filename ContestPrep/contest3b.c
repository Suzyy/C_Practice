...
void sort(int *board, int size);
void printArry(int *board, int size);
...
int getUniq(int *uniqBoard, int size)
{
  int nUniq;
  int last;
  int count = 0;
  ...
  for(int i = 0; i< size; i++)
  {
    ...
    else if(last != uniqBoard[i])
    {
      uniqBoard[count] = uniqBoard[i];
      count++;
    }
    last = uniqBoard[i];
  }
...
void getMaxLenAndNumFen(char* inFileName, int* actLongFen, int* actNumDiffH){
  ...
  int uniqBoardLen;
  uniqBoardLen = getUniq(uniqBoard, num); 
  
  printArry(uniqBoard, uniqBoardLen);
  
  int sumArry[uniqBoardLen * (uniqBoardLen - 1)/2];

  int sumCount = 0;

  for(int i = 0; i < uniqBoardLen-1; i++)
  {
    for(int j = i+1; j < uniqBoardLen; j++)
    {
      sumArry[sumCount] = (uniqBoard[i] + uniqBoard[j]);
      sumCount++;
    }
  }

  printArry( sumArry, sumCount);

  sumCount = getUniq(sumArry, sumCount);

  printArry( sumArry, sumCount);

  for(int i = 0; i < sumCount; i++)
  {
    int targetHeight = sumArry[i];
    int cLen = 0;
    int right = 0;
    for(int left = i; left < num; left++)
    {
      for(right = num-1; right > left; right--)
      {
        if(board[left] + board[right] == targetHeight)
        {
          cLen++;
